...
PROGRAM CGSolver
	...
	! Declare Domain Parameters
	kx = 100; ky = 100
	il = 2; jl = 2;
	ih = kx-1; jh = ky-1
	meshlx = 1.0; meshly = 1.0	
	rmax = 0.0001 	  
  gam = 100.0
	niter_precon = 5
	precon = .TRUE.
...
SUBROUTINE CGSolve(aw,ae,an,as,ap,b,t,rmax)
  ...
  DO WHILE (rcurrent.GT.rmax)		
  	...
  	! Get 'q' Matrix
  	CALL MatrixMultiply(aw,ae,an,as,ap,d,q)
  	
  	! Get 'gamma' = d^T*q
  	CALL DotProduct(d,q,gamma)
	...
	INTEGER :: i,j
		...
		DO j = jl,jh
			DO i = il,ih
				...
				m(i,j) = ( aw(i,j)*l(i-1,j) + ae(i,j)*l(i+1,j) + an(i,j)*l(i,j+1) &
								 + as(i,j)*l(i,j-1) + ap(i,j)*l(i,j) )
